...
function extractCompetitiveRank (p) {
  return p('.masthead-player .competitive-rank .h6').text()
}

function gatherAchievements (p) {
  const achievements = []
...
export function scrapePlayerStats (html) {
  ...
  stats.competitiveRank = extractCompetitiveRank(p)
  stats.achievements = gatherAchievements(p)
