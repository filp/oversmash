...
async function findPlayerStats (req, options, platform, region, name) {
  ...
  const stats = scrapePlayerStats(html, options.finderOverrides)
...
async function test () {
  ...
  // object (see below)
  const overwatch = main()
  console.log(util.inspect(await overwatch.playerStats('tonalddrump-21171', 'eu', 'pc'), { depth: 6 }))
}

test().then((err, data) => console.log(err, data))
