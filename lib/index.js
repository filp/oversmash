...
async function findPlayer (req, options, name) {
  ...
  const response = await req.get(searchPath, { responseType: 'json' })
...
async function findPlayerStats (req, options, platform, region, name) {
  ...
  const stats = scrapePlayerStats(options, html.data)
...
export default function main (callerOptions = {}) {
  ...
  // Prepare an instance of axios configured with requestOptions
  // as provided by the caller (or from defaultOptions)
  const req = axios.create(sharedOptions.requestOptions)
...
const ow = main()
ow.playerStats('tonalddrump-21171').then(console.log)
